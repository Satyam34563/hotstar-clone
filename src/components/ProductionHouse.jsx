    ...
    {productionHouseList.map((list)=>{
        return (
          <div className="border-[2px] border-gray-600 rounded-lg hover:scale-110 transition-all duration-300 ease-in-out cursor-pointer relative shadow-xl shadow-gray-800">
            <video className='absolute top-0 rounded-md opacity-0 hover:opacity-50 z-0' src={list.video} autoPlay loop playsInline muted></video>
            <img src={list.image} alt="" srcset="" className="w-full z-[1]" />
          </div>
        );
    })}
  </div>
